...
- 实时扫描
  ...
  - 支持计价币、15m/1h/2h/4h/8h/12h/1d 周期、候选池、成交额、成交笔数过滤
  - 按所选扫描周期自动组合 1H/2H/4H/8H/12H 连续放量或缩量，降低固定 24H 门槛造成的机会遗漏
- 技术指标
  - RSI(14)
  - EMA(20/50)
  - MACD(12, 26, 9)
...
扫描候选池默认开启多周期动态量能发现：先在受控发现池中读取 1H K 线，再根据所选扫描周期检查 1H/2H/4H/8H/12H 相邻窗口。默认成交额放量阈值为历史小时中位数的 `1.60x`，成交笔数阈值为 `1.25x`，至少连续命中两个窗口。固定 24H 门槛未达标时，只有实际 24H 活跃度达到分类门槛 `20%`、近期小时量折算 24H 达到分类门槛 `50%`，连续放量才可获得动态流动性准入。

动态准入只替代固定 24H 流动性门槛，评分、买压、反追高、支撑结构、波动率与仓位风控仍会执行。连续缩量只作为观察和风险提示，不会单独赋予自动交易资格。
