...
class AppSettings:
    ...
    min_trade_count: int = int(os.getenv("MIN_TRADE_COUNT", "30000"))
    btc_min_quote_volume: float = float(os.getenv("BTC_MIN_QUOTE_VOLUME", "150000000"))
    btc_min_trade_count: int = int(os.getenv("BTC_MIN_TRADE_COUNT", "150000"))
    eth_min_quote_volume: float = float(os.getenv("ETH_MIN_QUOTE_VOLUME", "100000000"))
    eth_min_trade_count: int = int(os.getenv("ETH_MIN_TRADE_COUNT", "120000"))
    xrp_min_quote_volume: float = float(os.getenv("XRP_MIN_QUOTE_VOLUME", "40000000"))
    xrp_min_trade_count: int = int(os.getenv("XRP_MIN_TRADE_COUNT", "60000"))
    sol_min_quote_volume: float = float(os.getenv("SOL_MIN_QUOTE_VOLUME", "60000000"))
    sol_min_trade_count: int = int(os.getenv("SOL_MIN_TRADE_COUNT", "80000"))
    bnb_min_quote_volume: float = float(os.getenv("BNB_MIN_QUOTE_VOLUME", "30000000"))
    bnb_min_trade_count: int = int(os.getenv("BNB_MIN_TRADE_COUNT", "50000"))
    top30_min_quote_volume: float = float(os.getenv("TOP30_MIN_QUOTE_VOLUME", "10000000"))
    top30_min_trade_count: int = int(os.getenv("TOP30_MIN_TRADE_COUNT", "30000"))
    dynamic_activity_enabled: bool = os.getenv("DYNAMIC_ACTIVITY_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
    activity_discovery_pool: int = int(os.getenv("ACTIVITY_DISCOVERY_POOL", "80"))
    activity_baseline_hours: int = int(os.getenv("ACTIVITY_BASELINE_HOURS", "48"))
    activity_surge_ratio: float = float(os.getenv("ACTIVITY_SURGE_RATIO", "1.6"))
    activity_trade_surge_ratio: float = float(os.getenv("ACTIVITY_TRADE_SURGE_RATIO", "1.25"))
    activity_contraction_ratio: float = float(os.getenv("ACTIVITY_CONTRACTION_RATIO", "0.65"))
    activity_trade_contraction_ratio: float = float(os.getenv("ACTIVITY_TRADE_CONTRACTION_RATIO", "0.75"))
    activity_min_consecutive_windows: int = int(os.getenv("ACTIVITY_MIN_CONSECUTIVE_WINDOWS", "2"))
    activity_liquidity_floor_ratio: float = float(os.getenv("ACTIVITY_LIQUIDITY_FLOOR_RATIO", "0.2"))
    activity_normalized_threshold_ratio: float = float(os.getenv("ACTIVITY_NORMALIZED_THRESHOLD_RATIO", "0.5"))
